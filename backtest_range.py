...
EARNINGS_POST_ONLY = "--post-only" in sys.argv  # Trueなら決算日0〜+N日のみ（=本番想定）。Falseで±N両側
...
def _load_earnings_calendar(days: int = EARNINGS_EXCLUSION_DAYS) -> None:
    ...
    EARNINGS_POST_ONLY=True なら決算後0〜+N日のみ（本番想定）、Falseなら±N日両側（look-ahead BT用）。
    """
    ...
    try:
        ...
        return
    start_offset = 0 if EARNINGS_POST_ONLY else -days
    mode_label = f"決算後0〜+{days}日のみ" if EARNINGS_POST_ONLY else f"決算前後±{days}日"
    total_excl = 0
    ...
    print(f"[backtest] 決算カレンダー: {len(_EARNINGS_EXCLUDED_DATES)}銘柄 / "
          f"延べ除外日{total_excl}件（{mode_label}）")
