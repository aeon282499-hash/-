...
戦略:
  ...
  エグジット : 以下のいずれか早い方
    1. 損切り  -3%（OCO・ザラ場ヒット）
    2. 利確    +5%（OCO・ザラ場ヒット）
    3. RSI回復  1〜2日目の終値で判定 → 翌営業日の寄り値で成売り（3日目は対象外）
    4. 最大保有 3営業日目の大引け（終値≒クロージングオークション成売り・強制）
        ...
        for ticker, full_df in all_data.items():
            ...
            try:
                ...
                for hold_day in range(1, MAX_HOLD + 1):
                    ...
                    # RSI回復チェック（終値ベース）— 最終日(MAX_HOLD)は強制決済のため対象外
                    if hold_day < MAX_HOLD:
                        ...
                        if rsi_exit:
                            ...
                            if next_idx >= len(all_trading_days):
                                break
                            next_date = all_trading_days[next_idx]
                            next_rows = full_df[full_df.index.strftime("%Y-%m-%d") == next_date]
                            if next_rows.empty:
                                break
                            ...
                            if next_open <= 0 or np.isnan(next_open):
                                break
                            if direction == "BUY":
                                pnl_pct = (next_open - entry_open) / entry_open * 100
                            else:
                                pnl_pct = (entry_open - next_open) / entry_open * 100
                            exit_date = next_date
                            exit_type = "RSI"
                            break
