...
戦略:
  エントリー : シグナル翌営業日の始値
  エグジット : 以下のいずれか早い方
    1. 損切り  ATR×1.5（銘柄ごとのボラティリティに適応）
    2. 利確    ATR×2.5（リスクリワード比 1:1.67）
...
ATR_STOP_MULT   = 1.5   # 損切り: ATR × この倍数
ATR_TP_MULT     = 2.5   # 利確:   ATR × この倍数
MAX_HOLD        = 5     # 最大保有営業日数
...
def run_range_backtest(start: str, end: str) -> None:
    ...
    print(f"  戦略: スイング（最大{MAX_HOLD}日・損切ATR×{ATR_STOP_MULT}・利確ATR×{ATR_TP_MULT}・RSI回復）")
    print(f"{'='*60}\n")
                ...
                for hold_day in range(1, MAX_HOLD + 1):
                    ...
                    if direction == "BUY":
                        if day_low <= stop_price:
                            pnl_pct   = (stop_price - entry_open) / entry_open * 100
                            exit_date = check_date
                            exit_type = "STOP"
                            break
                        if day_high >= tp_price:
                            pnl_pct   = (tp_price - entry_open) / entry_open * 100
                            exit_date = check_date
                            exit_type = "TP"
                            break
                    else:
                        if day_high >= stop_price:
                            pnl_pct   = (entry_open - stop_price) / entry_open * 100
                            exit_date = check_date
                            exit_type = "STOP"
                            break
                        if day_low <= tp_price:
                            pnl_pct   = (entry_open - tp_price) / entry_open * 100
                            exit_date = check_date
                            exit_type = "TP"
                            break
