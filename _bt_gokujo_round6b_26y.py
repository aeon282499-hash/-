...
# ════════════════════════════════════════════════════════
# 極上ラウンド6b: ★候補2本の深掘り
#  B' 貸借倍率(買残/売残)上位除外 … 本番同等の土台(買残回転≤1.2)で閾値/年別/噪音/買残回転との独立性
#  C' ATRサイズ正規化 … 平均係数=1に正規化(資金同額)して公平比較・年別・DD
# ════════════════════════════════════════════════════════
GV = G & V10
print("\n■ B' 貸借倍率の深掘り（2016-08〜・土台=極上∧買残回転≤1.2＝本番同等）", flush=True)
LIVE = GV & ~(np.isfinite(lturn) & (lturn > 1.2))
def line10b(lab, mask, base=None, noise=None):
    R = run10(mask); r = stat10(R); yy = R.groupby("y").yen.sum() / 1e4
    j = ""
    if base is not None:
        j = " ★" if (r["h1"] > base["h1"] and r["h2"] > base["h2"] and r["pf"] >= base["pf"] and r["worst"] >= base["worst"] - 1 and (noise is None or r["tot"] > noise)) else ""
    print(f"  {lab:<30} 玉{r['n']:>4} 勝率{r['win']:>5.1f}% PF{r['pf']:>5.2f} 10年{r['tot']:>+6,.0f}万 | 16-20{r['h1']:>+5,.0f} 21-26{r['h2']:>+5,.0f} | 勝年{r['wy']}/{r['ny']} 最悪{r['worst']:>+4,.0f}" + j + " | 年別 " + " ".join(f"{int(y)%100:02d}:{v:+.0f}" for y, v in yy.items()), flush=True); return r, yy
def nf10(base_mask, frac, seeds=30):
    idx = np.where(base_mask & V10)[0]; k = int(len(idx) * frac); tots = []
    for s in range(seeds):
        drop = rng.choice(idx, size=k, replace=False); m = base_mask.copy(); m[drop] = False; tots.append(stat10(run10(m))["tot"])
    return float(np.quantile(tots, 0.99)), float(np.std(tots))
print(f"  買残回転>1.2 の玉(本番では建てない) {int((GV & np.isfinite(lturn) & (lturn > 1.2)).sum())}/{GV.sum()}件")
Bl, yyb = line10b("土台(買残回転≤1.2)", G & ~(np.isfinite(lturn) & (lturn > 1.2)))
okr = LIVE & np.isfinite(ratio)
qs = np.nanquantile(ratio[okr], [0.2, 0.4, 0.6, 0.8]); print("  貸借倍率 五分位境界: " + "/".join(f"{q:.2f}" for q in qs))
edges = [-np.inf] + list(qs) + [np.inf]
for k in range(5):
    m = okr & (ratio > edges[k]) & (ratio <= edges[k + 1]); pm(f"五分位{k+1}", m)
for th in (3, 4, 5, 6, 8, 10, 15):
    m = G & ~(np.isfinite(lturn) & (lturn > 1.2)) & ~(np.isfinite(ratio) & (ratio > th)); frac = 1 - (LIVE & m).sum() / LIVE.sum(); nf, sd = nf10(G & ~(np.isfinite(lturn) & (lturn > 1.2)), frac)
    r, yy = line10b(f"貸借倍率>{th}を除外", m, Bl, nf); print(f"      除外率{frac*100:.0f}% 噪音99%点{nf:.0f}万 sd{sd:.0f} 年別で土台超え {int((yy.reindex(yyb.index).fillna(0) > yyb).sum())}/{len(yyb)}")
print("  買残回転×貸借倍率の独立性（件あたり%）:")
lt_lo = LIVE & np.isfinite(lturn) & (lturn <= np.nanmedian(lturn[LIVE])); lt_hi = LIVE & np.isfinite(lturn) & (lturn > np.nanmedian(lturn[LIVE]))
for lab_l, ml in (("買残回転 低", lt_lo), ("買残回転 高", lt_hi)):
    for lab_r, mr in (("倍率≤6.76", np.isfinite(ratio) & (ratio <= 6.76)), ("倍率>6.76", np.isfinite(ratio) & (ratio > 6.76))):
        pm(f"{lab_l}×{lab_r}", ml & mr)
print("  売残回転（分母を揃えた売り側単独・五分位）:")
oks = LIVE & np.isfinite(sturn); qs2 = np.nanquantile(sturn[oks], [0.2, 0.4, 0.6, 0.8]); e2 = [-np.inf] + list(qs2) + [np.inf]
for k in range(5): pm(f"売残回転 五分位{k+1}(≤{e2[k+1]:.3f})", oks & (sturn > e2[k]) & (sturn <= e2[k + 1]))

print("\n■ C' ATRサイズ正規化（26年・平均係数=1に正規化＝資金同額で公平比較）", flush=True)
ATRok = np.where(np.isfinite(ATRv) & (ATRv > 0), ATRv, np.nanmedian(ATRv[G]))
R0 = run_sized(G, np.ones(n)); pick_i = R0.i.to_numpy()
print(f"  極上玉のATR分布: 中央値{np.nanmedian(ATRv[pick_i]):.2f}% 平均{np.nanmean(ATRv[pick_i]):.2f}% 10%点{np.nanquantile(ATRv[pick_i],0.1):.2f} 90%点{np.nanquantile(ATRv[pick_i],0.9):.2f}")
def yrs(R): return (R.groupby("y").yen.sum() / 1e4)
r0 = line26("固定150万(現行)", R0); y0 = yrs(R0)
for ref in (2.0, 2.4, 2.8):
    for lo_, hi_ in ((0.7, 1.3), (0.5, 1.5), (0.5, 2.0)):
        mult = np.clip(ref / ATRok, lo_, hi_); mean_pick = mult[pick_i].mean(); mult_n = mult / mean_pick
        R = run_sized(G, mult_n); r = line26(f"ATR{ref} [{lo_},{hi_}] 正規化(÷{mean_pick:.2f})", R); y = yrs(R)
        print(f"      年別で現行超え {int((y.reindex(y0.index).fillna(0) > y0).sum())}/26 平均建玉{R.sh.mul(E0[R.i.to_numpy()]).mean()/1e4:.0f}万")
mult = np.clip(2.4 / ATRok, 0.7, 1.3); R = run_sized(G, mult); print(f"  参考: ATR2.4 [0.7,1.3] 非正規化 平均係数{mult[pick_i].mean():.2f} 平均建玉{R.sh.mul(E0[R.i.to_numpy()]).mean()/1e4:.0f}万")
