...
⚠️ 2026-09-09 監査: このスクリプトの「本番定義」は旧仕様（SIZE=50万・GAIN_MIN=6・株価300円下限・
丸め後判定・NO-GO混在順位）のまま＝現行 daytrade_paper.py（FADE_PX_MIN=0・raw判定・GO内順位・
①100万/②50万）のパリティ監査にはならない。出力の「BT vs 本番 -20%」は撤廃済みの300円下限が原因の
誤警報。現行のパリティは「実データを daily_top_fades に直接通して _bt_fade_26y_lib の式と突合」で
2026-08-20〜09-10 全日一致を確認済み（9/9 監査エージェント）。使うなら定義を現行に合わせてから。
