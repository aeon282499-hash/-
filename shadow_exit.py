...
GOKUJO_SIZE       = 1_500_000   # 2026-09-27 本人「縮小モードの間だけコードのサイズを150万に」＝9/24からの縮小モード(現金余力30万台)で本人が手で半分にしていた発注と配信株数をそろえる。
#   ★9/30の判定で現金余力55万以上なら 3_000_000 に戻す（本人決定の300万×1・R25で1×300万が資金効率最良）。旧: 3_000_000(9/21「ルール通り300万」)／1_500_000(9/19 R14)
...
# 26年候補のp75。26年+333→+469万 / PF1.28→1.47 / DD-88→-47 / 20年 玉勝率52.1→55.4%・勝ち年14→16/20。
# 閾値はp55〜p90の全域で改善（高原）なので±0.05程度のズレは効かない。None にすれば無効化。
# ⚠ 12ヶ月ローリングでマイナスが27ヶ月続いた期間がある（2020後半〜2022）。効かない時期で外さないこと。
GOKUJO_UPDN_MAX   = 1.091
...
def send_discord(today: date, key: str = "main") -> bool:
    ...
    if slot_full:
        ...
        # 損切/利確の行だけは同日の本人指示で復活（OCO発注に毎晩使う数字のため）。
        lines = [
