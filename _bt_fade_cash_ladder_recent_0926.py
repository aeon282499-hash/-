# -*- coding: utf-8 -*-
"""_bt_fade_cash_ladder_recent_0926.py — 期間を環境変数Y0で絞る版(直近でも②が危険を下げるか)。元: 現金余力C(30〜60万)から60営業日で15万割れ/ゼロを踏む率をサイズ別に(①②とも前終+1%指値・大型除外・10年)。
...
Y0 = int(os.environ.get("Y0", "2017")); Q = Q[Q.y >= Y0]
d = rank(Q); d = d[d.rk <= 2].reset_index(drop=True)
allday = pd.Series(sorted(set(P10[P10.y >= Y0].ent.astype(str))))
...
H = 60; rows = []
for s1, s2 in ((100, 50), (100, 0), (70, 50), (70, 0), (60, 0), (50, 50), (50, 25), (50, 0)):
    ...
    r = {"サイズ": f"①{s1}/②{s2}", "3か月後中央値万": round(np.median(e) / 1e4), "年平均万": round(ps.dot(sh) / 1e4 / (2026.67 - Y0))}
