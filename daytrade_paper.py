...
# 2026-08-15に 100万→70万（本人決定・一時措置）: 現金余力20万に対しフェード最悪1玉-33万は
# 「負けを払う現金」が足りない＝不足金リスク。70万なら最悪1玉約-23万・BT年約+90万ペース。
# **現金バッファが40万に届いたら100万へ復帰する約束**（決算持ち越しは同日実弾休止＝
# 実弾はフェード+スイング(極み)の2本体制・信用枠500万は全時間帯余裕）。
# 2026-08-05の 50万→100万（段階運用プラン: 8月=1番だけ・8月末在庫実測→9月2本目判断）は
# 生きている＝在庫実測は70万でも続く。10年BT(_bt_fade_100man_monthly.py・値がさ1万円プール):
...
# （70万なら7,000円以下）。旧50万採用の経緯(2026-07-28)はgit履歴参照。
CAPITAL_PER_TRADE = 700_000
EXPIRE_DAYS = 14
...
def send_monthly(book: dict, ym: str, dry: bool = False) -> bool:
    ...
    def _cap_of(month: str) -> int:
        ...
        # 8月はサイズ変更(8/5に100万・8/15に70万)を月中で跨ぐため月利%は概算（円は正確）。
