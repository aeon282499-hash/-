...
# RSI上限は2026-07-29に45→55へ緩和（_bt_earnings_ultra.py・クリーンデータでの10年検証）。
# 旧45で弾いていた帯（45-55）は単独でもPF1.30/1.18・両期間プラスだったため取り込む。
check("RSI上限は55", m.RSI_MAX == 55.0)
check("RSI55.0ちょうどは通過", m.rule_pass(55.0, -3.1, 1e9, 5000, 5000))
check("RSI55.1で除外", not m.rule_pass(55.1, -3.1, 1e9, 5000, 5000))
check("旧閾値の45.1も今は通過（緩和の意図どおり）", m.rule_pass(45.1, -3.1, 1e9, 5000, 5000))
check("runup-3.0ちょうどは除外(<厳密)", not m.rule_pass(40, -3.0, 1e9, 4000, 5000))
check("代金7.4億で除外", not m.rule_pass(40, -5, 7.4e8, 4000, 5000))
check("代金7.5億は通過(2026-07-11引き下げ)", m.rule_pass(40, -5, 7.5e8, 4000, 5000))
