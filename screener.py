...
■ BUYシグナル判定フロー（judge_signal_pre）
  ...
  ⑥ 除外         直近15営業日に「-5%超下落+出来高2倍」の決算系イベントがあればスキップ（PEAD回避）
...
# ── PEAD（決算後ドリフト）除外 ──
# 直近に「-5%以上の下落 + 出来高2倍」の日があれば BUY は15営業日スキップ。
# 「+5%以上の急騰 + 出来高2倍」の日があれば SELL は15営業日スキップ。
# 決算API依存ゼロで、業績下方修正・不祥事など悪材料下げを一網打尽に避ける。
...
def has_recent_event_move(df: pd.DataFrame, direction: str) -> bool:
    ...
    BUY: 下げイベントを検出（決算ミス/業績下方修正でのPEAD下方ドリフト回避）
    SELL: 上げイベントを検出（好決算サプライズでのPEAD上方ドリフト回避）
    """
...
def judge_signal_pre(ticker: str, name: str, df: pd.DataFrame) -> dict | None:
    ...
    if is_etf_ticker(ticker, name):
        return None
    if has_recent_event_move(df, "BUY"):
        return None
...
def run_screener() -> tuple[list[dict], list[dict], dict, list[dict], list[dict]]:
    ...
    for ticker, df in data.items():
        ...
        # BUY判定（judge_signal_pre 内で PEAD除外も実施）
        ...
        # SELL判定（judge_sell_signal_pre 内で PEAD除外も実施）
