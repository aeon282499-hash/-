...
# RSI(ewm)のウォームアップ用取得窓（暦日）。約20本の短窓ではRSIが±2〜4ptブレ、
# 窓の起点が1日ズレるだけで50境界の出口判定が割れる（2026-07-17 ビックカメラ:
# 朝run30日窓=48.0 / 週次ドライラン30日窓=50.4 / 長期真値=46.5 → 週次だけ誤決済表示）。
...
# 名称キーワードの誤爆で個別株を落としていた分の救済（2026-07-31の監査で全銘柄を走査して特定）。
# "ブル"/"ベア"/"ダブル" が社名の一部に含まれるだけの実在株が10銘柄あり、判定関数を
# 通す全システム（スイングの judge_signal_pre / judge_sell_signal_pre、各BT）で
# 10年間ずっと候補から消えていた。キーワード側を賢くしようとすると本物のETFを取りこぼす
# 危険があるので、**実在株を名指しで通す**という外科的な直し方にする。
# 影響は小さい（極みの10年BTで4回だけ選ばれ+3,842円／枠の押し出しまで含めて年+0.5万）が、
# ミネベアミツミのような主力が構造的に対象外なのは明確な誤りなので直す。
_NOT_ETF_TICKERS = {
    "2208.T",  # ブルボン
    "2798.T",  # ワイズテーブルコーポレーション
    "2804.T",  # ブルドックソース
    "3925.T",  # ダブルスタンダード
    "5597.T",  # ブルーイノベーション
    "6479.T",  # ミネベアミツミ
