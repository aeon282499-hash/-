...
■ BUYシグナル判定フロー（judge_signal_pre）
  ...
  ⑥ 除外         日経25MA以下のとき限定: 直近15営業日に「-5%下落+出来高2倍」の決算系イベントがあればスキップ（PEAD下方ドリフト回避）
...
ATR_VOL_CAP_BUY = 3.0            # 買いはBT(2026-05-07・243パターン検証)で2.5→3.0が最適 / 累積+468%→+640%

MAX_SIGNALS    = 5      # ※2026-04-29: 8→5 (1件100万・資金500万・MAX5件並列前提)
BATCH_SIZE     = 100   # yfinanceフォールバック用

# ── PEAD（決算系ショック後ドリフト）除外 ──
# BUY: 日経25MA以下（下降相場）のときのみON ※上昇相場ではPEAD後も反発しやすくフィルタが裏目（BT 2024-02〜2026-05でPF1.25→1.19悪化）
# SELL: 常時ON（SELLは元々日経25MA以下時のみ配信されるため）
PEAD_LOOKBACK_DAYS = 15
PEAD_DROP_PCT      = -5.0
PEAD_SPIKE_PCT     = 5.0
PEAD_VOL_MULT      = 2.0
...
def has_recent_event_move(df: pd.DataFrame, direction: str) -> bool:
    """直近PEAD_LOOKBACK_DAYS営業日に「±5%大変動 + 出来高2倍」の決算系イベント日があったか。
    df の末尾はシグナル判定日（=エントリー前日終値）。判定日自体も検出対象に含める。
    BUY: 下げイベント検出（業績下方修正・増資・決算ミス等のPEAD下方ドリフト回避）
    SELL: 上げイベント検出（好決算サプライズ等のPEAD上方ドリフト回避）
    """
    if len(df) < PEAD_LOOKBACK_DAYS + 21:
        return False
    daily_pct = (df["Close"] / df["Close"].shift(1) - 1) * 100
    vol_avg   = df["Volume"].rolling(20).mean().shift(1)
    vol_ratio = df["Volume"] / vol_avg
    chg_win  = daily_pct.iloc[-PEAD_LOOKBACK_DAYS:]
    volr_win = vol_ratio.iloc[-PEAD_LOOKBACK_DAYS:]
    if direction == "BUY":
        mask = (chg_win <= PEAD_DROP_PCT) & (volr_win >= PEAD_VOL_MULT)
    else:
        mask = (chg_win >= PEAD_SPIKE_PCT) & (volr_win >= PEAD_VOL_MULT)
...
def judge_signal_pre(ticker: str, name: str, df: pd.DataFrame) -> dict | None:
    ...
    ※ BUY側のPEAD除外は呼び出し側（日経MA以下時のみ適用）で実施。
    """
    ...
    # ── 高ボラ除外（買い: ATR/終値 > ATR_VOL_CAP_BUY%）─────
    atr = calc_atr(df)
    if atr is not None and last_close > 0:
        if (atr / last_close * 100) > ATR_VOL_CAP_BUY:
            return None
...
def judge_sell_signal_pre(ticker: str, name: str, df: pd.DataFrame) -> dict | None:
    ...
    ※ SELLは元々日経25MA以下時のみ配信されるため、PEAD-up検出は常時ON。
    """
    if has_recent_event_move(df, "SELL"):
        return None
    close = df["Close"].dropna()
    if len(close) < max(MA_DEV_PERIOD, ATR_PERIOD, 20) + 5:
        return None
...
def run_screener() -> tuple[list[dict], list[dict], dict, list[dict], list[dict]]:
    ...
    for ticker, df in data.items():
        ...
        # BUY判定（日経25MA以下=下降相場のときだけPEAD除外を適用：B案）
        result = judge_signal_pre(ticker, name, df)
        if result:
            if nk_above_ma25 is False and has_recent_event_move(df, "BUY"):
                print(f"  [SKIP PEAD] {ticker} 直近に決算系イベント下げ（下降相場のためスキップ）")
            else:
        ...
        # SELL判定（judge_sell_signal_pre 内でPEAD-up除外も実施）
