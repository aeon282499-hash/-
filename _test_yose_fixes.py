...
# --- RSIウォームアップ窓の回帰（2026-07-17 ビックカメラ週次誤決済の再発防止） ---
# RSI(ewm)は30日窓だと±2〜4ptブレ、窓起点が1日ズレるだけで50境界の出口判定が
# 割れる。tracker自前fetchの窓が screener.RSI_WARMUP_CAL_DAYS(>=90) であることを確認。
import screener as _scr  # noqa: E402

check("rsi窓: RSI_WARMUP_CAL_DAYS が90日以上", _scr.RSI_WARMUP_CAL_DAYS >= 90)

_captured_window = {}


def _capture_batch(token=None, start=None, end=None, tickers=None):
    _captured_window["start"] = start
    _captured_window["end"] = end
    return fake_batch(token, start, end, tickers)


tracker.batch_download_jquants = _capture_batch
tracker.update_positions([_mk_pending("BBBB.T", 1366)], TODAY)
tracker.batch_download_jquants = fake_batch  # 後続テスト用に戻す
from datetime import timedelta as _td  # noqa: E402

check("rsi窓: tracker自前fetchが120日窓で取得",
      _captured_window.get("start")
      == (TODAY - _td(days=_scr.RSI_WARMUP_CAL_DAYS)).strftime("%Y-%m-%d")
      and _captured_window.get("end") == TODAY.strftime("%Y-%m-%d"))
