...
【使い方】
  python backtest_breakout.py 2023-01-01 2025-12-31
"""
...
def run_backtest(start, end):
    ...
    print(f"\n{'='*60}")
    print(f"  ブレイクアウト・バックテスト期間: {start} 〜 {end}")
...
def _print_results(trades, equity_curve, start, end):
    ...
    out_trades = f"breakout_trades_{start}_{end}.csv"
    df_trades.to_csv(out_trades, index=False, encoding="utf-8-sig")
    print(f"\n  取引履歴を {out_trades} に保存しました")

    out_equity = f"breakout_equity_{start}_{end}.csv"
