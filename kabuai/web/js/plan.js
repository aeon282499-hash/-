...
function viewPlan() {
  const p = DATA.plan;
  if (!p) return `<h2>📋 作戦</h2><div class="card"><div class="empty">作戦データがまだありません（次のビルドで反映）。</div></div><p class="disc">${esc(DATA.disclaimer)}</p>`;
  const orders = p.orders || [], paper = p.paper || [], news = p.news || [], earn = p.earnings_tomorrow || [];
  const head = `<h2>📋 作戦 <span class="sub">${esc(p.target_date || "")} 分（${esc(p.date || "")} 引け・${esc(p.generated_at || "")} 生成）</span></h2>`;
  // ① 実弾の注文
  ...
  return head + ordersHtml + paperHtml + newsHtml + earnHtml + ladderHtml + `<p class="disc">${esc(DATA.disclaimer)}</p>`;
}
