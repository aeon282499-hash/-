...
出す物（全部「検証済みのルール」から機械的に組む。新しい予測は一切足さない）:
  ① 実弾の注文     … 🩳フェード GO（①100/②50・寄り成行）・💥崩壊◎（50万・寄指）・👑極上（150万×1・寄指・現金100万で2枠）・🔻極み売り（3×100万）
...
CRASH_SIZE = "50万"
GOKUJO_SIZE = "150万"   # 2026-09-19 夜 200万→150万・乗せなし（本人「1銘柄150万MAX・現金余力50万」・R14）。現金100万到達で2枠目(別銘柄150万)
...
def build_plan(rows: list[dict], sell_watch: dict | None, arena: dict | None, data_date: str,
    ...
    # ③ 👑極上（150万×1・寄指上限）/ 極み買い（紙）/ 🔻極み売り（3×100万）
    ...
    try:
        ...
        for fname, system, side, size, rule in (
                ("shadow_exit_gokujo.json", "👑極上", "買い", GOKUJO_SIZE, "3営業日目の大引けで売り／初日の引けが建値-1%以下なら翌朝処分／損切り-3%"),
