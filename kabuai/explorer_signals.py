...
機能:
  ...
  Phase4: 静かな初動（凪ブレイク・旧初動キャッチャーv6のアプリ集約 2026-07-09）
...
# ═══ Phase4: 静かな初動（凪ブレイク・立花型出来高理論） ═══════════
# 旧・初動キャッチャーv6(main_momentum.py/Discord配信)のアプリ集約(2026-07-09)。
# BT(bt_momentum_v6.py 再測定)=2024 PF1.16/2025 PF1.12/2026 PF0.83(直近不調)
# → 買い推奨ではなく探索用としてUIに正直表示する。

def nagi_break_recent(df: pd.DataFrame, cfg: dict) -> dict | None:
    """凪(低ボラ・動いてない)→出来高の静かな蓄積→20日高値を初突破した陽線。
    直近 recent_days 日以内に点灯していれば最新の点灯日を返す。"""
    ng = cfg["nagi"]
    if len(df) < 45:
        return None
    ...
    atr_pct = (tr.rolling(20).mean() / c) * 100
    ret20 = c / c.shift(20) - 1
    vol_ratio = v.rolling(5).sum() / (v.shift(5).rolling(15).mean() * 5 + 1e-9)
    brk = c > h.rolling(20).max().shift(1)
    trig = (atr_pct < float(ng["atr_max_pct"])) \
        & (ret20 >= float(ng["ret20_min"])) & (ret20 <= float(ng["ret20_max"])) \
        & (vol_ratio >= float(ng["vol_ratio_min"])) & (vol_ratio <= float(ng["vol_ratio_max"])) \
        & brk & (c > o) & (c * v >= float(ng["min_value"])) & (c >= float(ng["min_price"]))
    recent = trig.tail(int(ng["recent_days"])).fillna(False)
    if not bool(recent.any()):
        return None
    ts = recent[recent].index[-1]
    return {"date": ts.strftime("%Y-%m-%d"),
            "volr": round(float(vol_ratio.loc[ts]), 2),
            "atr_pct": round(float(atr_pct.loc[ts]), 1),
...
def build_explorer(data: dict, name_map: dict, data_date: str, cfg: dict | None = None) -> dict:
    ...
    for ticker, df in data.items():
        ...
        try:
            ...
            # ── 静かな初動（Phase4・凪ブレイク） ── 流動性は自前条件(min_value)で担保
            ngi = nagi_break_recent(df, cfg)
            if ngi:
